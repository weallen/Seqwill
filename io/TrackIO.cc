...
DataTypeEnum
TrackFile::GetTrackType(const std::string& trackname) 
{
    ...
    std::vector<std::string> subtracks = GetSubTrackNames(trackname);
    hid_t subtrack = H5Dopen2(trackgroup, subtracks[0].c_str(), H5P_DEFAULT);
    hid_t type = H5Dget_type(subtrack);
    if (type < 0) 
        return kUnknownType;
